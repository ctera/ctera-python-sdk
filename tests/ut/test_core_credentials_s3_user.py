...
from cterasdk.core.credentials import s3
from tests.ut import base_core_services


class TestCoreS3Credentials(base_core_services.BaseCoreServicesTest):
    ...
    def setUp(self):
        ...
        self._mock_get_session = self.patch_call("cterasdk.object.Portal.session")
        self._mock_get_session.return_value = munch.Munch({'user': munch.Mucnh({'name': self._username})})
        self._mock_get_user_uid = self.patch_call("cterasdk.core.users.Users.get")
        self._mock_get_user_uid.return_value = self._uid

    def test_list_s3_credentials(self):
        execute_response = 'Success'
        self._init_global_admin(execute_response=execute_response)
        ret = s3.S3(self._global_admin).all()
    ...
    def test_create_s3_credential(self):
        execute_response = 'Success'
        self._init_global_admin(execute_response=execute_response)
        ret = s3.S3(self._global_admin).create()
        self._mock_get_user_uid.assert_called_once_with(self._user_account, ['uid'])
    ...
    def test_delete_s3_credential_success(self):
        ...
        self._init_global_admin(execute_response=execute_response)
        ret = s3.S3(self._global_admin).delete(self._access_key_id, self._user_account)
    ...
    def test_delete_s3_credential_not_found(self):
        ...
        mock_get_s3_credentials = self.patch_call("cterasdk.core.credentials.S3.all")
        mock_get_s3_credentials.return_value = munch.Munch({'accessKey': self._access_key_id, 'uid': self._access_key_uid})
        ret = s3.S3(self._global_admin).delete('not_found')
        mock_get_s3_credentials.assert_called_once_with(self._user_account)
