...
class TestCoreGroups(base_core.BaseCoreTest):
    ...
    def test_get_local_group_success(self):
        ...
        self._global_admin.get_multi.assert_has_calls(
            [
                mock.call(f'/localGroups/{self._groupname}', mock.ANY),
                mock.call(f'/domains/{self._domain}/adGroups/{self._groupname}', mock.ANY)
            ]
        )
    ...
    def test_add_local_group(self):
        ...
        expected_param = self._get_group_object(name=self._groupname, description=self._description)
