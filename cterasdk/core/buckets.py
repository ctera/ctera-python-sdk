...
class Buckets(BaseCommand):
    ...
    def add(self, name, bucket, read_only=False, dedicated_to=None):
        ...
        logging.getLogger('cterasdk.core').info('Adding bucket. %s', {'name': name, 'bucket': bucket.bucket, 'type': bucket.__class__.__name__})
        response = self._core.api.add('/locations', param)
        logging.getLogger('cterasdk.core').info('Bucket added. %s', {'name': name, 'bucket': bucket.bucket, 'type': bucket.__class__.__name__})
        return response
