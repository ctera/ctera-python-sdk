...
def add_first_user(ctera_host, username, password, email):
    ...
    if info.isfirstlogin:
        ...
        logging.getLogger().info('User created. {0}'.format({'user' : username}))
...
def add(ctera_host, username, password, fullName, email, uid):
    ...
    try:
        ...
        logging.getLogger().info("User created. {0}".format({'username' : user.username}))
...
def delete(ctera_host, username):
    ...
    try:
        ...
        logging.getLogger().info("User deleted. {0}".format({'username' : username}))
