...
def add(ctera_host, name, directory, acl, access, csc, dirPermissions, comment, exportToAFP, exportToFTP, exportToNFS, exportToPCAgent, exportToRSync, indexed):
    ...
    try:
        ...
        logging.getLogger().info("Share created. {0}".format({'name' : name}))
...
def validate_root_directory(ctera_host, name):
    ...
    for root in response:
        ...
        if root.fullpath == ( '/%s' % name ):
            ...
            logging.getLogger().debug("Found root directory. {0}".format({'name' : root.name, 'type' : root.type, 'fullpath' : root.fullpath}))
            
            return name
        
    logging.getLogger().error("Could not find root directory. {0}".format({'name' : name}))
        
    options = [ root.fullpath[1:] for root in response ]
        
    raise InputError('Invalid root directory.', name, options)

def addShareACLRule(acls, type, name, perm):

    ace = Object()

    ace._classname = "ShareACLRule"
    ...
    ace.permissions._classname = "FileAccessPermissions"
...
def add_acl(ctera_host, name, acl):
    ...
    for entry in current_acl:
        ...
        ace_type = entry.principal2._classname
        ...
        if not entry_key in new_acl_dict:
...
def remove_acl(ctera_host, name, tuples):
    ...
    for entry in current_acl:
        ...
        if not (options.get(ace_type), ace_name) in tuples:
...
def delete(ctera_host, name):
    ...
    try: 
        ...
        logging.getLogger().info("Share deleted. {0}".format({'name' : name}))
