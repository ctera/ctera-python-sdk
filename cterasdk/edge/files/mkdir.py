...
def mkdir(ctera_host, path, recurse = False):
    ...
    if recurse:
        ...
        for i in range(1, len(array) + 1):
            ...
            except (ItemExists, Forbidden) as error:
...
def _mkdir(ctera_host, path):
    ...
    logging.getLogger().info('Creating directory. {0}'.format({'path' : fullpath}))
    
    try:
    
        response = ctera_host.mkcol(ctera_host._files(), fullpath)
        
    except CTERAException as error:
        
        _process_error(error, fullpath)
    
    logging.getLogger().info('Directory created. {0}'.format({'path' : fullpath}))
