...
from ..exception import CTERAException, InputError, ConnectionError
...
def connect(ctera_host, server, user, password, license):
    ...
    connect_to_services(ctera_host, param, license)
    
def activate(ctera_host, server, user, code, license):
...
def connect_to_services(ctera_host, param, license):
    ...
    try:
        ...
        logging.getLogger().error("Connection failed. Reason: " + description)
        
        raise CTERAException("Connection failed", None, reason = description)
    
    ctera_host.apply_license(license)
...
def validate_license(license):
    ...
    try:
        ...
        logging.getLogger().error('Connection failed. Invalid license type. {0}'.format({'license' : license}))
        
        raise error
    
def check_cttp_traffic(ctera_host, address, port = 995):
    
    tcp_conn_status = tcp_connect(ctera_host, address = address, port = 995)

    if not tcp_conn_status:

        logging.getLogger().error("Unable to establish connection over port " + str(port))
        
        raise ConnectionError('Unable to establish connection', None, host = address, port = port, protocol = 'CTTP')
    ...
    try:
        ...
        if obj.rc in ['UntrustedCert', 'UntrustedCA']:
            ...
            if config.connect['ssl'] == 'Consent':
                ...
                logging.getLogger().warn(msg = obj.msg)
...
def attach(ctera_host, param):
    ...
    logging.getLogger().info("Connecting to Portal. {0}".format({'server' : param.server, 'user' : param.user}))
