...
def terminate(Gateway):
    ...
    if session.local():
        ...
        logging.getLogger().debug('Terminating local session. {0}'.format({'host' : session.target_host(), 'user' : session.username()}))
    
        inactive_session(Gateway)
        
    elif session.remote():
        
        logging.getLogger().debug('Terminating remote session. {0}'.format({'host' : session.target_host(), 'tenant' : session.tenant(), 'user' : session.username()}))
...
class Session(Object):
    ...
    def __init__(self, host, type, status):
...
class ActiveSession(Session):
    ...
    def __init__(self, host, type, user):
        
        super().__init__(host, type, SessionStatus.Active)
