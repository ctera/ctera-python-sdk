...
def connect(ctera_host, domain, username, password, ou):
    ...
    if not tcp_conn_status:
        ...
        msg = "Connection failed. No traffic allowed over port " + str(port)

        logging.getLogger().error(msg = msg)
        
        raise ConnectionError('Unable to establish connection', None, host = host, port = port, protocol = 'LDAP')
    ...
    logging.getLogger().info("Connecting to Active Directory. {0}".format({'domain' : domain, 'user' : username}))
...
def advanced_mapping(ctera_host, domain, start, end):
    ...
    for mapping in mappings:
        ...
        if domain == mapping.domainFlatName:
            ...
            logging.getLogger().debug('Configuring advanced mapping. {0}'.format({'domain' : domain, 'start' : start, 'end' : end}))
            
            return ctera_host.put('/config/fileservices/cifs/idMapping/map', mappings)
            
    logging.getLogger().error('Could not find domain name. {0}'.format({'domain' : domain}))
